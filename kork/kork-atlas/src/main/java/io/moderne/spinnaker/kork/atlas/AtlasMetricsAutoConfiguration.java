...
/**
 * Wires Micrometer common tags and an Atlas-specific {@code baseUnit} tag
 * customizer for forked Spinnaker JVM services.
 *
 * <p>Atlas URI / step / batch size flow through Spring Boot's built-in
 * {@code management.atlas.metrics.export.*} keys. This module ships with
 * {@code enabled=false} as the default; the deploy mechanism enables
 * publishing by setting {@code MODERNE_ATLAS_URI} and
            ...
            public Meter.Id map(Meter.Id id) {
                ...
                if (unit == null || unit.isEmpty()) {
                    if (id.getType() == Meter.Type.TIMER
                            || id.getType() == Meter.Type.LONG_TASK_TIMER) {

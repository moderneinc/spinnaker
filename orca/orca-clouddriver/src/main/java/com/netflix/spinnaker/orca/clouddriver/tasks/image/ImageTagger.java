...
public abstract class ImageTagger {
  ...
  protected Collection findImages(
    ...
    if (imageNames == null || imageNames.isEmpty()) {
      ...
      for (String upstreamImageId : upstreamImageIds) {
        ...
        log.info(
            format(
                "Found upstream image '%s' (executionId: %s)",
                upstreamImageName, stage.getExecution().getId()));
      }
    }
      ...
      Map matchedImage =
          allMatchedImages.stream()
              ...
              .orElseThrow(
                  () ->
                      new ImageNotFound(
                          format("No image found (imageName: %s)", targetImageName),
                          !upstreamImageIds.isEmpty()));
                  ...
                  String cloudProvider =
                      (String)
                          stage
                              .getContext()
                              .getOrDefault(
                                  "cloudProvider", stage.getContext().get("cloudProviderType"));
                  boolean consideredStageRefIdMatches =
                      consideredStageRefIds == null
                          || consideredStageRefIds.isEmpty()
                          || consideredStageRefIds.contains(stage.getRefId())
                          || (stage.getParent() != null
                              && consideredStageRefIds.contains(stage.getParent().getRefId()));
                  return consideredStageRefIdMatches
                      && (stage.getContext().containsKey("imageId")
                          || stage.getContext().containsKey("amiDetails"))
                      && cloudProviderType.equals(cloudProvider);
                })
            .collect(toList());

    return imageProvidingAncestorStages.stream()
        .map(
            it -> {
              if (it.getContext().containsKey("imageId")) {
                ...
                return (String)
                    ((List<Map>) it.getContext().get("amiDetails")).get(0).get("imageId");
              }
            })
        .collect(toList());
  }
  ...
  /**
   * This method is a helper for AmazonImageTagger; Amazon images are regional with a one-to-many
   * relationship between image names (treated globally) and regional image ids. Clouddriver caches
   * Amazon images and namedImages as distinct and eventually consistent collections but
   * findImages() uses the output of a lookup by names to determine which imageIds to tag. If
   * upstream images were baked in n regions for "myapp-1.0.0" but findImage("aws", "myapp-1.0.0")
   * returns n-1 amis as the namedImages collection for an account/region is mid-update,
   * UpsertImageTags would appear successful despite only applying tags to a subset of upstream
   * images.
   *
   ...
   *     <p>Throws ImageNotFound with shouldRetry=true in AmazonImageTagger.foundAllImages
   */

...
class EnableAzureServerGroupAtomicOperationUnitSpec extends Specification {
  ...
  void "resumes automatic repairs after enabling a server group that has them enabled"() {
    given:
    ...
    when:
    new EnableAzureServerGroupAtomicOperation(description).operate([])

    then:
    1 * networkClient.enableServerGroupWithAppGateway(RESOURCE_GROUP, RESOURCE_GROUP, "app-gateway", SERVER_GROUP_NAME, "backend-pool")

    then:
    1 * computeClient.resumeAutomaticRepairs(RESOURCE_GROUP, SERVER_GROUP_NAME)
  }

  void "resumes automatic repairs when the server group is already enabled"() {
    given:
    ...
    then:
    0 * networkClient.enableServerGroupWithAppGateway(*_)
    1 * computeClient.resumeAutomaticRepairs(RESOURCE_GROUP, SERVER_GROUP_NAME)
  }

  void "does not touch automatic repairs when the server group does not have them enabled"() {
    given:
    computeClient.getServerGroup(RESOURCE_GROUP, SERVER_GROUP_NAME) >> appGatewayServerGroup(disabled: true, automaticRepairsEnabled: false)

    when:
    new EnableAzureServerGroupAtomicOperation(description).operate([])

    then:
    1 * networkClient.enableServerGroupWithAppGateway(*_)
    0 * computeClient.resumeAutomaticRepairs(*_)
  }

  void "does not resume automatic repairs when enabling fails"() {
    given:
    computeClient.getServerGroup(RESOURCE_GROUP, SERVER_GROUP_NAME) >> appGatewayServerGroup(disabled: true, automaticRepairsEnabled: true)
    networkClient.enableServerGroupWithAppGateway(*_) >> { throw new RuntimeException("Azure API error") }
  ...
  void "fails the operation when resuming automatic repairs fails"() {
    given:
    computeClient.getServerGroup(RESOURCE_GROUP, SERVER_GROUP_NAME) >> appGatewayServerGroup(disabled: true, automaticRepairsEnabled: true)
    computeClient.resumeAutomaticRepairs(*_) >> { throw new RuntimeException("Azure API error") }
